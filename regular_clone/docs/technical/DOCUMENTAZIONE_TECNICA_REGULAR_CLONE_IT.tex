...
1.6 & 2026-03-23 & Team progetto & Inserita sezione esplicita sui valori sperimentali assunti pre-calibrazione (fenologia, coefficenti genetici/famiglia, bounds calibrazione twin) e riorganizzazione path repository (\texttt{docs/*}, \texttt{economics/*}). \\
...
(\texttt{yield\_cap\_annual\_kg}, default 80.0) convertito internamente in
\texttt{kg/m2/year} tramite \texttt{farm\_active\_area\_m2}.\\
...
\begin{enumerate}
  ...
  \item si applica un cap hard di produzione annua per mantenere la farm entro il budget/obiettivo commerciale;
...
Knowledge & \texttt{core/genetics.py}, \texttt{core/literature.py}, \texttt{configs/*.json} & Profili genetici, priors cultivar, registry fonti, override bounds/coefficenti. \\
...
\newpage
\chapter{Parametrizzazione Fenologica e Spazio Decisionale}
...
\section{Valori sperimentali assunti (baseline pre-calibrazione)}
Questa sezione esplicita i valori assunti dal sistema prima della calibrazione su dati reali.
In questa fase tali valori sono \textbf{priors sperimentali} o vincoli hard runtime.

\subsection*{A) Vincoli family-hard indica/sativa}
\begin{longtable}{@{}p{4.2cm}p{2.1cm}p{2.1cm}p{4.5cm}@{}}
\toprule
Parametro & Indica & Sativa & Fonte / uso \\ \midrule
\texttt{clone\_cycle\_days\_from\_cutting} & 105 & 120 & \texttt{core/model.py}, vincolo hard runtime. \\
\texttt{stage\_days\_from\_cutting} & 12/24/7/32/30 & 12/27/7/37/37 & \texttt{core/model.py}, non ottimizzabile. \\
\texttt{flower\_density\_pl\_m2} & 4.0 & 2.0 & \texttt{core/model.py}, \texttt{core/production\_planner.py}. \\
\texttt{seed\_germination\_days} & 10--14 & 10--14 & \texttt{SEED\_GERMINATION\_DAYS\_RANGE}. \\
\texttt{seed\_to\_cutting\_days} & 27--35 & 27--35 & metadata profilo, lead time pre-ciclo. \\
\bottomrule
\end{longtable}

\subsection*{B) Coefficienti sperimentali di modello (profilo regular)}
\begin{longtable}{@{}p{4.3cm}p{2.0cm}p{2.0cm}p{2.0cm}@{}}
\toprule
Coefficiente & Base & Fam. indica & Fam. sativa \\ \midrule
\texttt{yield\_potential\_scale} & 1.05 & 0.99 & 1.02 \\
\texttt{quality\_potential\_scale} & 1.03 & 0.98 & 1.03 \\
\texttt{disease\_pressure\_scale} & 0.95 & 0.97 & 1.03 \\
\texttt{hlvd\_pressure\_scale} & 0.95 & 1.00 & 1.01 \\
\texttt{nutrient\_window\_scale} & 1.02 & 1.04 & 0.98 \\
\texttt{flower\_duration\_scale} & n/a & 0.94 & 1.08 \\
\texttt{dli\_demand\_scale} & n/a & 0.95 & 1.08 \\
\bottomrule
\end{longtable}
Valori caricati da \texttt{configs/genetic\_profiles.json}; rappresentano priors sperimentali pre-calibrazione.

\subsection*{C) Bounds parametri di calibrazione twin}
\begin{longtable}{@{}p{4.4cm}p{2.0cm}p{2.0cm}p{4.4cm}@{}}
\toprule
Parametro \texttt{TwinCalibration} & Min & Max & Significato \\ \midrule
\texttt{yield\_gain\_scale} & 0.60 & 1.60 & scala guadagno resa giornaliera \\
\texttt{quality\_gain\_scale} & 0.60 & 1.60 & scala guadagno qualita giornaliera \\
\texttt{transpiration\_scale} & 0.60 & 1.60 & scala traspirazione \\
\texttt{disease\_inc\_scale} & 0.50 & 2.00 & incremento rischio patogeni \\
\texttt{hlvd\_inc\_scale} & 0.50 & 2.00 & incremento pressione HLVD \\
\texttt{penalty\_scale} & 0.50 & 2.50 & scala penalita processo \\
\texttt{energy\_scale} & 0.60 & 1.80 & scala consumo energetico \\
\texttt{yield\_post\_scale} & 0.70 & 1.30 & scala resa ciclo finale \\
\texttt{quality\_post\_scale} & 0.70 & 1.30 & scala qualita ciclo finale \\
\texttt{quality\_post\_offset} & -20.0 & 20.0 & offset qualita finale \\
\bottomrule
\end{longtable}
Questi intervalli sono definiti in \texttt{calibrate\_twin.py} e costituiscono il dominio di fit nella fase di calibrazione.
...
In tutte le modalita e attivo il vincolo hard \texttt{yield\_cap\_annual\_kg} (farm-level),
tradotto in soglia \texttt{yield\_cap\_kg\_m2\_year} tramite \texttt{farm\_active\_area\_m2}.
...
\begin{lstlisting}
.\.venv\Scripts\python.exe .\optimizer_literature_best.py --mode both --json-only
\end{lstlisting}
...
.\.venv\Scripts\python.exe .\controller_literature_realtime.py ^
  --mode max_yield --source serial_json --serial-port COM4 --serial-baud 115200 ^
  --watchdog-timeout-s 5 --yield-cap-annual-kg 80 --farm-active-area-m2 120 ^
  --store-db .\runtime\db\cea_timeseries.db
\end{lstlisting}
...
\begin{lstlisting}
.\.venv\Scripts\python.exe .\controller_literature_realtime.py ^
  --mode max_yield_energy --energy-cap-kwh-m2 640 ^
  --yield-cap-annual-kg 80 --farm-active-area-m2 120 ^
...
\end{lstlisting}
Output principale: KPI annui (ricavi, EBITDA, ROI, payback, break-even) e check di coerenza con planner.
...
\texttt{test\_e2e\_optimizer.py} & Run modes optimizer, metriche derivate clone cycle, varianti energy. \\
\texttt{test\_e2e\_calibration.py} & E2E CLI calibrazione twin su dataset sintetico. \\
\texttt{test\_realtime\_safety.py} & Fallback setpoint, emissione fallback tick, limiti hard/rate. \\
...
\texttt{test\_production\_planner.py} & Vincoli planner 80 kg/anno, check saturazione e rispetto cap annuo da schedule settimanale. \\
\texttt{test\_economic\_hybrid\_model.py} & Verifica modulo economico locale \texttt{regular\_clone/economics/cea\_economic\_analysis.py}. \\
\bottomrule
\end{longtable}
...
\begin{enumerate}
  ...
  \item \texttt{regular\_clone/configs/genetic\_profiles.json}. Profili genetici e override bounds/coefficenti.
