...
HARD_STAGE_DAYS_BY_FAMILY = {
    # MODIFICATO: ciclo talea regular hardcoded per famiglia, senza liberta' sulla durata totale
    ...
    # MODIFICATO: hybrid mantenuta ma non-default come interpolazione operativa
    # fonte: scelta progettuale allineata a documento tecnico v2.5
    "hybrid": {
        "propagation": 12,
        "vegetative": 26,
        "transition": 7,
...
CLONE_CYCLE_TARGET_DAYS = {
    family: int(sum(stage_days.values())) for family, stage_days in HARD_STAGE_DAYS_BY_FAMILY.items()
}

FLOWER_DENSITY_BY_FAMILY = {
    # MODIFICATO: densita' hard regular per famiglie produttive indica/sativa
    # fonte: Team Progetto CEA (2026) BUISNESS PLAN (1)
    "indica_dominant": 4.0,
    "sativa_dominant": 2.0,
    "hybrid": 3.0,
...
CUTTINGS_PER_VEGETATIVE_PLANT = 5.0
# MODIFICATO: vincoli hard pre-ciclo seme -> talea (business plan)
# fonte: Team Progetto CEA (2026) BUISNESS PLAN (1)
SEED_GERMINATION_DAYS_RANGE = (10.0, 14.0)
SEEDLING_TO_CUTTING_DAYS_RANGE = (17.0, 21.0)
PROPAGATION_DAYS_DEFAULT = 12
TRANSITION_DAYS_DEFAULT = 7
...
class StrategyBuilder:
    ...
    @staticmethod
    def _clone_cycle_target_days(cultivar_family: str) -> int:
        return int(CLONE_CYCLE_TARGET_DAYS.get(cultivar_family, CLONE_CYCLE_TARGET_DAYS["hybrid"]))

    @staticmethod
    def _hard_stage_days(cultivar_family: str) -> Dict[str, int]:
        base = HARD_STAGE_DAYS_BY_FAMILY.get(cultivar_family, HARD_STAGE_DAYS_BY_FAMILY["hybrid"])
        return {stage: int(base[stage]) for stage in STAGE_ORDER}

    @staticmethod
    def _flower_density_pl_m2(cultivar_family: str) -> float:
        return float(FLOWER_DENSITY_BY_FAMILY.get(cultivar_family, FLOWER_DENSITY_BY_FAMILY["hybrid"]))
    ...
    def build(self, p: Dict[str, float], mode: str) -> StrategyProfile:
        ...
        hard_stage_days = self._hard_stage_days(self.cultivar_family)
        # MODIFICATO: ciclo da talea hard per famiglia; veg/flower_days non alterano piu' la durata totale
        # fonte: Team Progetto CEA (2026) BUISNESS PLAN (1)
        propagation_days = hard_stage_days["propagation"]
        transition_days = hard_stage_days["transition"]
        veg_days = hard_stage_days["vegetative"]
        flower_early_days = hard_stage_days["flower_early"]
        flower_late_days = hard_stage_days["flower_late"]
        flower_total_days = flower_early_days + flower_late_days
        target_total_days = int(sum(hard_stage_days.values()))
        ...
        stage_days = {
            "propagation": propagation_days,
            "vegetative": veg_days,
            "transition": transition_days,
        ...
        flower_density = self._flower_density_pl_m2(self.cultivar_family)
        veg_density = flower_density
        seed_to_cutting_days_min = SEED_GERMINATION_DAYS_RANGE[0] + SEEDLING_TO_CUTTING_DAYS_RANGE[0]
        seed_to_cutting_days_max = SEED_GERMINATION_DAYS_RANGE[1] + SEEDLING_TO_CUTTING_DAYS_RANGE[1]
        ...
        metadata: Dict[str, float | str | bool] = {
            ...
            "veg_density_pl_m2": veg_density,
            "flower_density_pl_m2": flower_density,
            "business_plan_clone_cycle_target_days": float(target_total_days),
            "business_plan_clone_cycle_target_months": float(target_total_days / 30.0),
            "business_plan_cycle_hardcoded_from_cutting": True,
            "business_plan_cuttings_per_plant": CUTTINGS_PER_VEGETATIVE_PLANT,
            "business_plan_cuttings_capacity_pl_m2": veg_density * CUTTINGS_PER_VEGETATIVE_PLANT,
            "business_plan_no_dedicated_mother_plants": True,
            "business_plan_cuttings_source": "vegetative_plants_in_production_cycle",
            "business_plan_seed_germination_days_min": SEED_GERMINATION_DAYS_RANGE[0],
            "business_plan_seed_germination_days_max": SEED_GERMINATION_DAYS_RANGE[1],
            "business_plan_seed_to_cutting_days_min": seed_to_cutting_days_min,
            "business_plan_seed_to_cutting_days_max": seed_to_cutting_days_max,
...
def clone_cycle_derived_metrics(
    profile: StrategyProfile,
    dry_yield_g_m2: float,
    energy_kwh_m2: float,
) -> Dict[str, float | str]:
    cycle_days = max(int(sum(int(profile.stage_days.get(stage, 0)) for stage in STAGE_ORDER)), 1)
    raw_density = profile.metadata.get("flower_density_pl_m2", 0.0)
    try:
        flower_density_pl_m2 = float(raw_density)
    except (TypeError, ValueError):
        flower_density_pl_m2 = 0.0
    flower_density_safe = max(flower_density_pl_m2, 1e-9)
    cycles_per_year = 365.0 / float(cycle_days)
    dry_yield_g_plant_cycle = float(dry_yield_g_m2) / flower_density_safe
    return {
        "cycle_days": float(cycle_days),
        "flower_density_pl_m2": float(flower_density_pl_m2),
        "dry_yield_g_m2_cycle": float(dry_yield_g_m2),
        "dry_yield_g_plant_cycle": float(dry_yield_g_plant_cycle),
        "dry_yield_kg_m2_year": float((float(dry_yield_g_m2) / 1000.0) * cycles_per_year),
        "dry_yield_kg_plant_year": float((dry_yield_g_plant_cycle / 1000.0) * cycles_per_year),
        "energy_kwh_m2_cycle": float(energy_kwh_m2),
        "energy_kwh_m2_day_avg": float(float(energy_kwh_m2) / float(cycle_days)),
        "energy_kwh_m2_year": float(float(energy_kwh_m2) * cycles_per_year),
        "units_version": "v1",
    }
...
def reference_audit_summary(source_rows: List[Dict[str, object]]) -> Dict[str, int]:
    summary = {"compatible": 0, "incompatible": 0, "ambiguous": 0}
    for row in source_rows:
        cls = str(row.get("compatibility_regular", "")).strip().lower()
        if cls in {"yes", "compatible"}:
            summary["compatible"] += 1
        elif cls in {"no", "incompatible"}:
...
def run_mode(mode: str, args: argparse.Namespace) -> Dict:
    ...
    density_flower = float(derived["flower_density_pl_m2"])
    per_plant = float(derived["dry_yield_g_plant_cycle"])
    cycle_days = float(derived["cycle_days"])
    outcome_dict = outcome_to_dict(best.outcome, logs_tail=6)
    ...
    return {
        ...
        "clone_cycle_days": cycle_days,
        "flower_density_pl_m2": density_flower,
        "energy_kwh_m2_cycle": best.outcome.energy_kwh_m2,
