...
Baseline attuale: codice pronto per pre-calibrazione con `yield_target_annual_kg` parametrico (default `80`) e baseline startup low-cost selezionata su `startup_low_capex + grid_only_retrofit + core_efficiency_extended`.

## Priorita immediate readiness (upgrade verso 9.5+/10)

1. `R1 - Calibrazione reale su banco pilota 1-2 m2 (bloccante)`
- Eseguire almeno un ciclo osservato completo con dataset calibrazione pieno e coerente.
- Output richiesto: `runtime/profiles/twin_calibration.json` validato con loss stabile e metriche coerenti per famiglia.

2. `R2 - Commissioning HIL completo sensori/attuatori (bloccante)`
- Eseguire test hardware-in-the-loop in condizioni reali con fault injection controllata (timeout, outlier, salti sensore, fallback).
- Output richiesto: evidenza log di gestione corretta `fallback/source_error` e stabilita dei setpoint comandati.

3. `R3 - Congelamento parametri economici locali e rilancio KPI ufficiali (bloccante pre-go-live)`
- Bloccare tariffa energia locale, listino fornitori, costo manodopera e manutenzione sulla base contrattuale reale.
- Rilanciare `scripts/run_economic_analysis.py` con i valori congelati e aggiornare i KPI ufficiali pre-go-live.

Dipendenze readiness: `R1 -> R2 -> R3`.
...
- Per le modalita resa, imporre `quality_floor` (default `62`) per evitare profili ad alta resa ma qualita insufficiente.
- Mantenere `yield_target_annual_kg` parametrico (default `80`, modificabile a target reale).

9. `D2 - Verifica vincoli hard (codice)`
- Controllare nel profilo finale:
  - energia ciclo <= `energy_cap_kwh_m2`
  - resa annua target in uguaglianza: `projected_annual_yield_kg = yield_target_annual_kg`
...
- Se disponibile, includere telemetria energetica opzionale (`power_total_kw`, `power_led_kw`, `power_hvac_kw`, `power_pumps_kw`) per audit costi.
...
- `yield_target_annual_kg`: default `80`, sempre parametrico (alias CLI legacy: `yield_cap_annual_kg`).
- `energy_cap_kwh_m2` / `energy_cap_kwh_m2_cycle`: cap energetico per ciclo.
- `quality_floor`: soglia qualita minima in modalita `max_yield`/`max_yield_energy` (default `62`).
- Densita hard family:
  - indica `4.0` pl/m2
  - sativa `2.0` pl/m2
